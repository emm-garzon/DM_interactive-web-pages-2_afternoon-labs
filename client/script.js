...
*/
let stringValue = "Turtle";

const repeatMyParam = (event) => {
  axios
    .get(`http://localhost:3000/repeat/${stringValue}`)
    .then((response) => {
...
const queryString = "Pizza";

const giveMeInfo = (event) => {
  axios
    .get(`http://localhost:3000/query-test?${queryString}`)
    .then((response) => {
      console.log(response.data);
    })
    .catch((error) => {
...
queryButton.addEventListener("click", giveMeInfo);
...
/*
    ...
    -- In the console is displays: You sent an empty query!

    2: Send more than 1 query on the URL -- what happened?

    -- In the console it displays: You sent query: "pizza","pizza" with value: !
*/
...
const createFood = (event) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      newParaTwo.textContent = response.data;
